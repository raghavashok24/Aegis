...
app.post('/analyze', async (req, res) => {
  ...
  try {
    ...
    // SumSub has no list-all-applicants endpoint.
    // Fetch ALL transactions in the review window, group by applicantId to discover entities.
    async function getAllTxnsInWindow(from, to) {
      ...
      while (true) {
        try {
          const r = await sumsubRequest('GET', `/resources/kyt/txns?createdAtGt=${f}&createdAtLt=${t}&limit=${limit}&offset=${offset}`);
          const items = r.items || r.list || [];
          txns.push(...items);
    ...
    // Fetch current period transactions and group by applicant
    const allWindowTxns = await getAllTxnsInWindow(reviewStart, reviewEnd);
    const applicantTxnMap = {};
    for (const txn of allWindowTxns) {
      const aid = txn.applicantId || txn.applicant_id;
      if (!aid) continue;
      if (!applicantTxnMap[aid]) applicantTxnMap[aid] = [];
      applicantTxnMap[aid].push(txn);
    }
    const applicants = Object.keys(applicantTxnMap).map(id => ({ id, applicantId: id }));
    ...
    for (let i=0; i<applicants.length; i+=BATCH) {
      ...
      const batchRes = await Promise.allSettled(batch.map(async app => {
        ...
        // Use pre-fetched txns for current period, fetch baseline separately
...
// Backend URL is this same page's origin — no config needed
...
function exportSummary(results) {
  const lines=[
    ...
    ...results.entityResults.flatMap(e=>e.flags.filter(f=>f.severity==='High').flatMap(f=>[
      ...
      '  '+f.id+' — '+f.rule, '  '+f.rationale, f.sarRisk?'  ⚠ SAR RISK':''
    ]).filter(Boolean))
  ];
...
function exportCSV(results) {
  const hdr=['Flag ID','Period','Applicant','Rule','Category','Severity','Score','Tier','SAR Risk','Action','Rationale'];
  const rows=[hdr.join(',')];
...
function App() {
  ...
  const onRun=async(mode)=>{
    ...
    try{
      const r=await fetch(BACKEND+'/analyze',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({mode})});
