...
app.post('/analyze', async (req, res) => {
  ...
  try {
    ...
    function detectFlags(aid, txns, cur, bl, hist) {
      ...
      if (txns.length >= 4) {
        ...
        for (let i = 1; i < wkArr.length; i++) {
          if (wkArr[i] >= wkArr[i-1] * 2 && wkArr[i-1] > 0) {
            push('Velocity Escalation', 'Spec §4.1 | Rule 2', 'Behavioral Deviation', 'Medium', 'Weekly cadence doubled: ' + wkArr[i-1] + '→' + wkArr[i] + ' tx. Layering initiation indicator.', ids(txns).slice(0, 10), [], false);
            break;
          }
        }
      }
      ...
      if (out.length >= 3) {
        ...
        Object.entries(bmap).forEach(([party, a]) => {
          if (total > 0 && a / total > 0.7)
            push('Concentration Risk — Single Beneficiary', 'Spec §4.1 | Rule 5', 'Behavioral Deviation', 'High',
              (a / total * 100).toFixed(1) + '% of outbound to "' + party + '" — exceeds 70% threshold.',
      ...
      if (txns.length >= 5) {
        ...
        if (rnd.length / txns.length > 0.4)
          push('Round-Dollar Clustering', 'Spec §4.1 | Rule 19', 'Behavioral Deviation', 'Medium',
            (rnd.length / txns.length * 100).toFixed(1) + '% of txns (' + rnd.length + '/' + txns.length + ') at exact round amounts — exceeds 40%.',
            ids(rnd).slice(0, 15), [], false);
      }

      if (hist.length >= 2 && hist.slice(-2).every(p => p.txnCount < 3) && cur.txnCount >= 5 && cur.totalVolume >= 25000)
        push('Dormancy-to-Activity Transition', 'Spec §4.1 | Rule 9', 'Behavioral Deviation', 'High',
          'Dormant (<3 tx/period) for 2 prior periods, now ' + cur.txnCount + ' tx/$' + cur.totalVolume.toLocaleString('en', {maximumFractionDigits:0}) + ' — both thresholds exceeded.',
          ids(txns).slice(0, 10), [], false);
        ...
        if (tv > 0) {
          Object.entries(mmap).forEach(([m, v]) => {
            if (v / tv > 0.6) {
              ...
              push('Temporal Concentration — Single Month', 'Spec §4.1 | Rule 14', 'Behavioral Deviation', 'Medium',
                (v / tv * 100).toFixed(1) + '% of quarterly volume in ' + mn + ' — exceeds 60% threshold.',
      ...
      if (si > 0.2)
        push('Structuring — Just-Below-Threshold', 'Spec §4.2 | Rule 4 | 31 U.S.C. § 5324', 'Structuring & Layering', 'High',
          'Structuring Index ' + (si * 100).toFixed(1) + '%: ' + sub.length + '/' + txns.length + ' txns in $8K–$9,999 band. Federal offense under 31 U.S.C. § 5324.',
          ids(sub), [], true);

      const sorted = [...sub].sort((a, b) => new Date(a.createdAt || 0) - new Date(b.createdAt || 0));
      for (let i = 0; i < sorted.length - 1; i++) {
        const hr = (new Date(sorted[i+1].createdAt || 0) - new Date(sorted[i].createdAt || 0)) / 3600000;
        if (hr <= 72) {
          push('Structuring — 72h Window', 'Spec §4.2.1 | 31 U.S.C. § 5324', 'Structuring & Layering', 'High',
            'Sub-threshold txns within ' + hr.toFixed(1) + 'h. FATF Rec. 20 structuring indicator.',
            ids(sorted.slice(i, i + 3)), [], true);
          break;
        }
      }

      if (txns.length >= 4) {
        const amts = txns.map(t => amt(t)).filter(a => a > 0);
        if (amts.length >= 4) {
          const m = amts.reduce((a, b) => a + b, 0) / amts.length;
          const maxV = Math.max(...amts.map(a => Math.abs(a - m) / m));
          if (maxV < 0.05)
            push('Frequency & Uniformity Pattern', 'Spec §4.2.1 | Rule 4', 'Structuring & Layering', 'Medium',
              txns.length + ' txns with <5% variance (max ' + (maxV * 100).toFixed(2) + '%). Deliberate splitting indicator.',
      ...
      Object.entries(bgrp).forEach(([party, grp]) => {
        ...
        const tot = grp.reduce((s, t) => s + amt(t), 0);
        const mul = Math.round(tot / 10000) * 10000;
        if (Math.abs(tot - mul) <= 500 && mul > 0)
          push('Split Payments — Same Beneficiary', 'Spec §4.2.1 | 31 U.S.C. § 5324', 'Structuring & Layering', 'High',
            grp.length + ' payments to "' + party + '" = $' + tot.toLocaleString('en', {maximumFractionDigits:0}) + ' — within $500 of $' + mul.toLocaleString() + ' multiple.',
            ids(grp), [party], true);
      });
      ...
      Object.entries(inMap).forEach(([party, inA]) => {
        ...
        if (ratio <= 0.2) {
          ...
          push(w7 ? 'Reciprocal Payment — Escalated (7d)' : 'Reciprocal Payment Pattern', 'Spec §4.3.1 | Rule 11', 'Network & Flow Pattern', w7 ? 'High' : 'Medium',
            'Bidirectional with "' + party + '": in $' + inA.toLocaleString('en', {maximumFractionDigits:0}) + ' / out $' + outA.toLocaleString('en', {maximumFractionDigits:0}) + ' — ' + (ratio * 100).toFixed(1) + '% diff.' + (w7 ? ' Within 7 days — pre-coordination.' : ''),
        ...
        out.forEach(outT => {
          ...
          if (hrs >= 0 && hrs <= 48) {
            ...
            if (net >= 0 && net <= 0.1 && ip !== op)
              push('Pass-Through / Transit Payment', 'Spec §4.3.2 | Rule 3', 'Network & Flow Pattern', 'High',
                '$' + inA.toLocaleString('en', {maximumFractionDigits:0}) + ' from "' + ip + '" to "' + op + '" in ' + hrs.toFixed(1) + 'h at ' + (net * 100).toFixed(1) + '% retention. Conduit entity.',
                [inT, outT], [ip, op], true);
          }
        });
      });
      ...
      if ((tIn + tOut) > 50000 && tIn > 0 && tOut > 0) {
        const r = Math.abs(tIn - tOut) / Math.max(tIn, tOut);
        if (r <= 0.1)
          push('Quarterly Net Flow Near-Zero', 'Spec §4.3 | Rule 3', 'Network & Flow Pattern', 'High',
            'In $' + tIn.toLocaleString('en', {maximumFractionDigits:0}) + ' / Out $' + tOut.toLocaleString('en', {maximumFractionDigits:0}) + ' — ' + (r * 100).toFixed(1) + '% diff on >$50K. Pass-through conduit.',
            ids(txns).slice(0, 10), [], true);
      }
    ...
    function makeRFI(flag, applicantName) {
      ...
      const sidList = flag.txnIds.filter(Boolean).slice(0, 8).join(', ');
      const tidList = flag.txnInternalIds.filter(Boolean).slice(0, 8).join(', ');
      const why = 'FLAG: ' + flag.id + ' — ' + flag.rule + '\nREF: ' + flag.ruleRef + '\nPERIOD: ' + periodLabel + '\n\nDETECTION RATIONALE:\n' + flag.rationale + '\n\nSumSub Transaction IDs: ' + (sidList || 'N/A') + (tidList ? '\nInternal Transaction IDs: ' + tidList : '') + sarWarn;
...
<style>
:root{--ink:#0D1117;--ink-mid:#3D4451;--ink-muted:#64748B;--surface:#F7F8FA;--panel:#FFF;--border:#E2E5EA;--accent:#1A56DB;--accent-lo:#EEF3FF;--red:#C0392B;--red-lo:#FDF2F2;--amber:#D97706;--amber-lo:#FFFBEB;--green:#16A34A;--green-lo:#F0FDF4;--purple:#7C3AED;--purple-lo:#F5F3FF;--mono:'SF Mono','Fira Code',monospace;}
...
.tids{font-family:var(--mono);font-size:11px;background:var(--ink);color:#94A3B8;padding:10px 14px;border-radius:6px;margin-bottom:12px;}
.tid-row{display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap;}
.tid-label{color:#475569;font-size:10px;width:68px;flex-shrink:0;text-transform:uppercase;letter-spacing:.3px;}
...
.month-grid{display:grid;grid-template-columns:repeat(6,1fr);gap:6px;margin-bottom:10px;}
.mbtn{padding:7px 4px;border-radius:6px;border:1px solid var(--border);font-size:12px;cursor:pointer;background:transparent;color:var(--ink-mid);transition:all .15s;}
.mbtn:hover{background:var(--accent-lo);border-color:var(--accent);}
.mbtn.sel-m{background:var(--accent);color:#fff;border-color:var(--accent);font-weight:500;}
.mbtn:disabled{color:var(--border);cursor:not-allowed;}
.mode-tabs{display:flex;gap:6px;margin-bottom:14px;}
.mtab{padding:6px 14px;border-radius:6px;border:1px solid var(--border);font-size:12px;font-weight:500;cursor:pointer;background:transparent;color:var(--ink-mid);transition:all .15s;}
...
function exportSummary(results) {
  const lines = [
    ...
    ...results.entityResults.flatMap(e => e.flags.filter(f => f.severity === 'High').flatMap(f => [
      ...
      '  ' + f.id + ' — ' + f.rule,
      '  ' + f.rationale,
      '  SumSub IDs: ' + (f.txnIds || []).filter(Boolean).slice(0,8).join(', '),
...
function exportCSV(results) {
  const hdr = ['Flag ID','Period','Applicant ID','Applicant Name','Rule','Category','Severity','Score','Tier','SAR Risk','Action','SumSub Txn IDs','Internal Txn IDs','Counterparties','Rationale'];
  const rows = [hdr.join(',')];
  results.entityResults.forEach(e => e.flags.forEach(f => rows.push([
    f.id, results.periodLabel, e.applicantId,
...
function FlagCard({flag, applicantName}) {
  ...
  const sumsubIds = (flag.txnIds || []).filter(Boolean);
  const internalIds = (flag.txnInternalIds || []).filter(Boolean);

  const dlRFI = async () => {
    try {
      const r = await fetch(BACKEND + '/export/rfi', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({flag, applicantName})});
    ...
    <div className="fc">
      ...
      {open && (
        <div className="fb">
          ...
          {(sumsubIds.length > 0 || internalIds.length > 0) && (
            <div className="tids">
              <div style={{color:'#475569',fontSize:10,textTransform:'uppercase',letterSpacing:'.4px',marginBottom:8}}>Transaction IDs</div>
              {sumsubIds.slice(0, 8).map((sid, i) => (
                <div key={i} className="tid-row">
                  <span className="tid-label">SumSub</span>
                  <a href={'https://cockpit.sumsub.com/checkus#/kyt/transaction/' + sid} target="_blank" rel="noopener noreferrer" style={{color:'#93C5FD',textDecoration:'underline',fontFamily:'var(--mono)',fontSize:11,flex:1}}>{sid}</a>
                  {internalIds[i] && (
                    <span style={{display:'flex',alignItems:'center',gap:6}}>
                      <span className="tid-label" style={{width:'auto'}}>Internal</span>
                      <span style={{color:'#CBD5E1',fontSize:11}}>{internalIds[i]}</span>
                    </span>
                  )}
                </div>
              ))}
              {sumsubIds.length === 0 && internalIds.slice(0, 8).map((tid, i) => (
                <div key={i} className="tid-row">
                  <span className="tid-label">Internal</span>
                  <span style={{color:'#CBD5E1',fontSize:11}}>{tid}</span>
                </div>
              ))}
              {sumsubIds.length > 8 && <div style={{color:'#475569',marginTop:4}}>+ {sumsubIds.length - 8} more</div>}
            </div>
          )}
          {flag.rfi && (
            <div className="rfw">
...
function Dashboard({results, onRun, loading, msg, error}) {
  ...
  const [selYear, setSelYear] = useState(prevMonthYear);
  const [selMonths, setSelMonths] = useState([prevMonth]);

  const years = [];
  for (let y = curYear; y >= 2024; y--) years.push(y);

  const toggleMonth = m => {
    setSelMonths(prev => prev.includes(m) ? prev.filter(x => x !== m) : [...prev, m].sort((a,b) => a-b));
  };

  const buildPayload = () => {
    if (mode === 'monthly') return {mode: 'monthly'};
    if (mode === 'quarterly') return {mode: 'quarterly'};
    if (!selMonths.length) return null;
    const sorted = [...selMonths].sort((a,b) => a-b);
    const pad = n => String(n).padStart(2, '0');
    return {mode: 'custom', customStart: selYear + '-' + pad(sorted[0]), customEnd: selYear + '-' + pad(sorted[sorted.length-1])};
  };

  const handleRun = () => { const p = buildPayload(); if (p) onRun(p); };
  ...
  return (
    <div>
      <div className="card">
        ...
        <div className="mode-tabs">
          {[['custom','Select Months'],['monthly','Prior Month'],['quarterly','Prior Quarter']].map(([v,lbl]) => (
            <button key={v} className={'mtab' + (mode === v ? ' active' : '')} onClick={() => setMode(v)}>{lbl}</button>
          ))}
        </div>
        ...
        {mode === 'custom' && (
          <div style={{marginBottom:14}}>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
              ...
              <select className="sel" value={selYear} onChange={e => setSelYear(Number(e.target.value))}>
                {years.map(y => <option key={y} value={y}>{y}</option>)}
              </select>
              <span style={{fontSize:11,color:'var(--ink-muted)'}}>
                {selMonths.length === 0 ? 'Select months below' : selMonths.length === 1 ? MONTHS[selMonths[0]-1] + ' ' + selYear : selMonths.length + ' months selected'}
              </span>
              {selMonths.length > 0 && (
                <button className="btn bg bsm" onClick={() => setSelMonths([])} style={{marginLeft:'auto',fontSize:11}}>Clear</button>
              )}
            </div>
            <div className="month-grid">
              {MONTHS.map((mn, i) => {
                const m = i + 1;
                const isSel = selMonths.includes(m);
                const isFuture = selYear === curYear && m > curMonth;
                return (
                  <button key={m} onClick={() => !isFuture && toggleMonth(m)} disabled={isFuture} className={'mbtn' + (isSel ? ' sel-m' : '')}>{mn}</button>
                );
              })}
            </div>
            {selMonths.length > 1 && (
              <div style={{fontSize:11,color:'var(--ink-muted)',marginTop:4}}>
                Analysing {MONTHS[selMonths[0]-1]} to {MONTHS[selMonths[selMonths.length-1]-1]} {selYear}
              </div>
            )}
          </div>
        )}
        ...
        <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
          <button className="btn bp" onClick={handleRun} disabled={loading || (mode === 'custom' && selMonths.length === 0)}>
          ...
          {mode === 'custom' && selMonths.length === 0 && <span style={{fontSize:11,color:'var(--amber)'}}>Select at least one month</span>}
        </div>
      ...
      {!results && !loading && (
        <div className="empty"><div className="ei">🔍</div><p>No analysis run yet.</p><p className="eh">Select a period and click Run Analysis.</p></div>
      )}
        ...
        <div>
              ...
              <tbody>
                {(results.top5 || []).map(e => (
                  <tr key={e.applicantId}>
                    ...
                    <td>{e.sarRiskCount > 0 ? <span className="badge br">{'SAR ' + e.sarRiskCount}</span> : <span className="badge bg2">None</span>}</td>
                  </tr>
                ))}
          ...
          <div className="card">
            ...
            <div className="mani">
              ...
              <div><span className="mk">{'runtime:      '}</span><span className="mv">{(results.runtimeMs || 0).toLocaleString() + 'ms'}</span></div>
            </div>
          </div>
        </div>
      )}
...
function FlagsTab({results}) {
  ...
  const all = results.entityResults.flatMap(e => e.flags.map(f => ({...f, _name: e.applicantName, _score: e.riskScore, _tier: e.riskTier})));
  const filtered = all.filter(f =>
    (sev === 'All' || f.severity === sev) &&
    (cat === 'All' || f.category === cat) &&
    (sar === 'All' || f.sarRisk) &&
    (!q || f.rule.toLowerCase().includes(q.toLowerCase()) || f._name.toLowerCase().includes(q.toLowerCase()))
  );
...
function App() {
  ...
  const MSGS = ['Fetching transactions from SumSub…','Building statistical baselines…','Running behavioral deviation rules…','Running structuring detection…','Analyzing network patterns…','Computing risk scores…','Generating RFIs…','Finalizing…'];
  let mi = 0;
  ...
  const sarCount = results ? results.sarRiskFlags : 0;
      ...
      <div className="sidebar">
        ...
        <nav className="nav">
          ...
          <div className={'ni' + (tab === 'flags' ? ' active' : '')} onClick={() => setTab('flags')}>
            🚩 Flags
            {results && results.totalFlags > 0 && <span className="badge ba" style={{marginLeft:'auto',fontSize:10}}>{results.totalFlags}</span>}
          </div>
        </nav>
        <div className="sf">
          <div style={{color: ok === true ? '#86EFAC' : ok === false ? '#FCA5A5' : '#94A3B8'}}>
            <span className={'dot ' + (ok === true ? 'dok' : ok === false ? 'derr' : 'didle')} />
            {ok === true ? 'Ready' : ok === false ? 'Check env vars' : 'Connecting…'}
          </div>
