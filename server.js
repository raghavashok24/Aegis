...
app.post('/analyze', async (req, res) => {
  ...
  try {
    ...
    function normalizeTxn(txn) {
      ...
      return {
        id: txn.id,
        txnId: d.txnId || txn.id,
        applicantId: txn.applicantId,
        externalUserId: txn.externalUserId,
        createdAt: d.txnDate || txn.createdAt,
        direction: d.info?.direction || '',
        amount: d.info?.amount || d.info?.amountInDefaultCurrency || 0,
        fiatAmount: d.info?.amountInDefaultCurrency || d.info?.amount || 0,
        currencyCode: d.info?.currencyCode || '',
        counterpartyName: d.counterparty?.fullName || d.counterparty?.externalUserId || '',
        counterparty: d.counterparty?.externalUserId || '',
        beneficiaryName: d.counterparty?.fullName || '',
        senderName: d.applicant?.fullName || '',
      };
    }
